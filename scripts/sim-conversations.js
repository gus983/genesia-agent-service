...
const PERSONAS = [
...

  {
    ...
    market: 'PE',
    max_turns: 6,
    scenario: 'Médico ginecólogo de Lima, Perú. Pregunta por precios del NIPT para sus pacientes y si puede pagar en dólares.',
    user_style: 'Peruano, cordial. Pregunta por opciones y diferencias entre paneles. Quiere saber tiempos de resultado.',
    expected_behaviors: [
      'Valeria usa precios en soles (S/) correctamente para PE',
      'Confirma que también se puede pagar en USD',
      'Informa el centro en Lima (César Vallejo, Lince)',
      'Diferencia tiempos de resultado: 7-15 días (esenciales) vs 5-10 días (premium)',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 6,
    scenario: 'Médico que pregunta si el NIPT funciona para embarazo gemelar. Su paciente tiene gemelos dicigóticos.',
    user_style: 'Técnico, directo. Quiere saber exactamente qué paneles aplican y cuáles no.',
    expected_behaviors: [
      'Valeria aclara que Standard Pro, Advanced, Advanced Pro y Genome NO son aptos para embarazo gemelar',
      'Standard y MaterniT21 Plus SÍ son aptos',
      'No confunde las restricciones entre paneles',
      'No inventa compatibilidades',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 6,
    scenario: 'Médico que pregunta si puede derivar una paciente que actualmente está en semana 8 de embarazo.',
    user_style: 'Pragmático. Quiere saber si puede mandar a la paciente ya o tiene que esperar.',
    expected_behaviors: [
      'Valeria explica que el turno se puede coordinar desde semana 6',
      'La toma en sí es desde semana 9 (premium) o 10 (esencial)',
      'Sugiere coordinar el turno ahora y programarlo para cuando llegue a semana 9-10',
      'No confunde "cuando se puede coordinar" con "cuando se puede tomar"',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 7,
    scenario: 'Paciente embarazada de 32 años que se contacta directamente (no derivada por médico). Pregunta cuánto cuesta y si es seguro para el bebé.',
    user_style: 'Preguntas simples, lenguaje no técnico. Un poco nerviosa. Usa "nene" en lugar de "feto".',
    expected_behaviors: [
      'Valeria ajusta el lenguaje para una paciente (no usa tecnicismos)',
      'Aclara que es no invasivo y seguro',
      'Sugiere consultar con su obstetra',
      'No menciona honorarios médicos en ningún momento',
      'No asume que es médico',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 5,
    scenario: 'Médico que pregunta si su paciente puede pagar en efectivo y en pesos. Está en Mar del Plata.',
    user_style: 'Práctico. Pregunta por detalles de pago y logística local.',
    expected_behaviors: [
      'Valeria informa que se acepta efectivo SOLO en la oficina central CABA (Olleros 2411)',
      'Informa el centro de Mar del Plata (Laboratorio AC, Belgrano 3388) con costo adicional',
      'Aclara que el pago en pesos usa el tipo de cambio BNA venta (sin dar el número)',
      'No contradice: si la paciente está en MdP no puede pagar efectivo en el centro local',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 6,
    scenario: 'Médico que pregunta qué pasa si el resultado del NIPT es positivo para T21. Quiere saber el flujo para su paciente.',
    user_style: 'Clínico, quiere entender el protocolo post-resultado positivo.',
    expected_behaviors: [
      'Valeria aclara que NIPT es SCREENING, no diagnóstico',
      'Un resultado positivo no confirma la condición — se requiere diagnóstico confirmatorio (amniocentesis, biopsia vellosidades)',
      'No garantiza resultados ni da porcentajes de exactitud sin verificarlos en KB',
      'Escala si la pregunta requiere orientación clínica específica que no está en KB',
    ],
  },
  {
    ...
    market: 'AR',
    max_turns: 8,
    scenario: 'Médico que en una misma conversación pregunta por: precios, logística en Rosario, si hay descuento para volumen, y al final pregunta por los honorarios.',
    user_style: 'Habla rápido, mezcla temas en el mismo mensaje. No espera que Valeria haga preguntas.',
    expected_behaviors: [
      'Valeria responde un tema a la vez sin abrumar',
      'Informa el centro de Rosario (Instituto de Fertilidad Colabianchi, Oroño 1520)',
      'Para descuentos por volumen: escala, no inventa condiciones',
      'Activa gate de honorarios cuando llega ese tema',
      'No pierde el hilo entre los distintos temas',
    ],
  },
];
